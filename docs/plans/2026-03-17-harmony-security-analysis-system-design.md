...
### 4.4 aisearch-mcp-server (HTTP 传输)

| 工具名称 | 职责 | 优先级 |
|---------|------|--------|
| `chatCompletions` | 百度千帆联网搜索和智能总结 | P1 |

**说明**: 此 MCP Server 使用 HTTP 传输协议，直接连接百度千帆平台的 AI 搜索服务，支持 Agent 进行联网搜索获取实时信息。
...
| **MCP Servers** | 4个 (3个 stdio + 1个 HTTP) | 数据获取、知识查询、报告存储、联网搜索 |
